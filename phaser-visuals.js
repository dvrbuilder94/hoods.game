// Hoods visual overhaul v0.7 — richer RPG town dressing and layered ground.
(() => {
function wait(){
...
function install(scene){
  ...
  // Cobblestone town square texture.
  ground.fillStyle(0x77766f,.92).fillRoundedRect(640,570,610,330,18);
  for(let y=586,row=0;y<888;y+=22,row++){
    for(let x=654+(row%2?11:0);x<1230;x+=34){
      const n=((x*13+y*7)%19);ground.fillStyle(n<6?0x6c6c66:n<12?0x818078:0x74736c,.82).fillRoundedRect(x,y,28,16,4);
    }
  }
  // Crossroads blend into square.
  ...
  // Market/plaza border stones.
  ground.lineStyle(4,0x504f4a,.95).strokeRoundedRect(640,570,610,330,18);
  // Small gardens.
  const garden=(x,y,w,h)=>{
    ground.fillStyle(0x465a35,.95).fillRoundedRect(x,y,w,h,8);ground.lineStyle(3,0x3b3329,.9).strokeRoundedRect(x,y,w,h,8);
    for(let yy=y+10;yy<y+h-6;yy+=18)for(let xx=x+10;xx<x+w-6;xx+=20){const k=(xx+yy)%4;ground.fillStyle([0xc77965,0xd3b552,0xb07aac,0xe5d2a0][k],.85).fillCircle(xx,yy,2.4)}
  };
  garden(665,600,105,48);garden(1030,600,105,48);garden(665,845,115,38);garden(1020,845,115,38);
  // Lamps and barrels as foreground-capable world props.
  const lamp=(x,y)=>{
    const c=scene.add.container(x,y).setDepth(Math.floor(y/10));
    c.add(scene.add.rectangle(0,4,8,40,0x3a332c));c.add(scene.add.rectangle(0,-18,18,20,0x4b4439).setStrokeStyle(2,0x241f1b));
    c.add(scene.add.rectangle(0,-18,9,11,0xe0a84f,.92));c.glow=scene.add.circle(0,-18,24,0xe0a84f,.08);c.addAt(c.glow,0);
    scene.tweens.add({targets:c.glow,alpha:{from:.05,to:.14},duration:900+Phaser.Math.Between(0,350),yoyo:true,repeat:-1});return c;
  };
  ...
  // Zone labels styled more like world signs.
