// Hoods V2.2 — clean vertical slice with flat full-body character atlas.
(() => {
const VERSION='v2.2';
...
class HoodsV2 extends Phaser.Scene {
 ...
 create(){
   ...
   this.cameras.main.setZoom(this.scale.width<=700?1.16:1.05);
 ...
 paintWorld(d){
   ...
   const prop=(kind,x,y)=>{const gid=GID[kind];if(gid==null)return;const layer=(kind==='tree'||kind==='lamp')?decorTop:(kind==='flower'?groundBorders:items);layer.putTileAt(gid,x,y)};
   ...
   d.buildings.forEach(b=>{
     ...
     roof.setAlpha(.96);if(b.roofTint)roof.setTint(b.roofTint);this.roofZones.push({b,roof,inside:false});
   });
 }
 installCollisions(d){
 ...
 makeNpcs(d){
   ...
   (d.npcs||[]).forEach(n=>{
     ...
     const label=this.add.text(x,y-31,n.name,{fontFamily:'monospace',fontSize:'8px',color:'#efe8cf',stroke:'#171a14',strokeThickness:3}).setOrigin(.5).setDepth(24);
 ...
 update(time){
   ...
   this.player.setDepth(20+this.player.y/1000);this.playerName.setPosition(this.player.x,this.player.y-31).setDepth(this.player.depth+2);this.playerShadow.setPosition(this.player.x,this.player.y+10).setDepth(this.player.depth-2).setScale(moving?1+.03*Math.sin(time/90):1,1);
   const ts=this.tileSize;this.roofZones.forEach(r=>{const [x,y,w,h]=r.b.rect,inside=this.player.x>(x+.35)*ts&&this.player.x<(x+w-.35)*ts&&this.player.y>(y+.35)*ts&&this.player.y<(y+h-.35)*ts;if(inside!==r.inside){r.inside=inside;this.tweens.killTweensOf(r.roof);this.tweens.add({targets:r.roof,alpha:inside?.06:.96,duration:120})}});
 }
}
