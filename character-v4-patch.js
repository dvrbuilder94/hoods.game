// Hoods Character v4 — real sprite-atlas renderer generated from the approved character sheet.
(()=>{
 const boot=()=>{
  ...
  const install=()=>{
   ...
   s.drawHumanoid=function(c,{name='Hoods',dir='down',frame=0,gear={},npc=false}={}){
    ...
    const shadow=this.add.ellipse(0,27,31,8,0x050605,.32);
    c.add(shadow);
    const spr=this.add.sprite(0,-8,'charsV4',idx).setOrigin(.5,.5).setScale(.40);
    spr.setTexture('charsV4',idx);
    c.add(spr);
    // Keep equipment as overlays so the shared anatomy never changes.
    if(!npc&&dir!=='up'){
  ...
  s.load.spritesheet('charsV4','assets/v08/characters-v4.png?v=1040',{frameWidth:192,frameHeight:192,endFrame:27});
