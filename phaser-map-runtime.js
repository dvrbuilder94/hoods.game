// Hoods map migration P4 — Town core + west art chunk, incremental and fallback-safe.
(()=>{
const CORE_KEY='town-core',CORE_URL='maps/town/town-core.json?v=map-p4',TILE_KEY='town-basic',TILE_URL='assets/maps/tiles/town-basic.svg?v=map-p4';
const WEST_KEY='town-west',WEST_URL='maps/town/town-west.json?v=map-p4',WEST_TILE_KEY='hoods-town-v1',WEST_TILE_URL='assets/maps/tiles/hoods-town-v1.svg?v=map-p4';
...
function installCore(scene){
 ...
 try{
  ...
  console.info('[Hoods map] P4 core loaded',chunk.meta,'collision objects',collisionBodies.length);
...
function loadWest(scene){
 ...
 const haveMap=scene.cache.tilemap.exists(WEST_KEY),haveTiles=scene.textures.exists(WEST_TILE_KEY);
 if(haveMap&&haveTiles)return ready();
 if(!haveMap)scene.load.tilemapTiledJSON(WEST_KEY,WEST_URL);
 if(!haveTiles)scene.load.svg(WEST_TILE_KEY,WEST_TILE_URL);
 scene.load.once('complete',ready);
...
function installWest(scene){
 ...
 try{
  ...
  const tiles=map.addTilesetImage('hoods-town-v1',WEST_TILE_KEY,32,32,0,0);if(!tiles)throw new Error('tileset hoods-town-v1 not available');
  ...
  window.HoodsMaps=window.HoodsMaps||{zones:{}};window.HoodsMaps.zones.town=scene.__hoodsTownChunks;
  console.info('[Hoods map] P4 west art loaded',chunk.meta,'collision objects',collisionBodies.length);
