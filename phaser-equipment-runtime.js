// Hoods equipment UI v1 — Tibia-style set display; gear affects stats, outfits stay separate.
(()=>{
...
function inspectGear(root){
 ...
 for(const row of root.querySelectorAll('.p-item')){
  ...
  if(status==='UNEQUIP'||status==='EQUIPPED')gear[slot]={name,button:row,rarity:inferRarity(row,small)};
 }
 return gear;
}
...
function decorate(root){
 ...
 const copy=document.createElement('div');copy.className='hoods-set-copy';copy.innerHTML='<strong>Tibia-style equipment</strong>Gear lives here and changes <b>stats</b>. Your full-body outfit/skin controls how the character looks in the world.';
 shell.append(side,copy);
 const heading=document.createElement('div');heading.className='hoods-bag-heading';heading.textContent='BACKPACK / ITEMS';
...
function install(scene){
 ...
 console.info('[Hoods equipment] set UI mounted; world appearance remains outfit-driven');
}
