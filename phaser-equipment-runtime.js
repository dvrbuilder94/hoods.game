// Hoods equipment UI v1.2 — Tibia-style set display with visible gear icons; outfits stay separate.
(()=>{
...
function decorateRows(root){
 for(const row of root.querySelectorAll('.p-item')){
  ...
  const name=row.querySelector('b')?.textContent?.trim()||'',slot=ITEM_SLOT[name];if(!slot)return;
...
function decorate(root){
 ...
 const copy=document.createElement('div');copy.className='hoods-set-copy';copy.innerHTML='<strong>Equipment set</strong>Items sit in the set like classic MMORPG equipment. Gear changes <b>stats</b>; the selected outfit controls the body skin.';
 ...
 root.prepend(heading);root.prepend(shell);decorateRows(root);
}
function install(scene){
 ...
 scene.renderInventory();scene.renderShop();
 console.info('[Hoods equipment] v1.2 mounted with visible set and item icons');
}
